...
# Integrated AST tools imports
import shutil
from src.github_fetcher import fetch_repo, RepoFetchError
from src.import_scanner import scan_package_usage
from src.sandbox_checker import simulate_upgrade as run_sandbox_upgrade
from src.impact_analyzer import build_api_map, diff_packages
from src.smart_scanner import scan_project_usages
from src.ai_remediation import generate_remediation
...
async def run_pipeline(args: argparse.Namespace) -> None:
    ...
    github_url_source = getattr(args, "github_url", None)
    if github_url_source:
        try:
            print_stat_row("GitHub Fetcher", f"Cloning {github_url_source}...", style="cyan")
            project_dir = await fetch_repo(github_url_source)
        except Exception as exc:
            log.error("Failed to clone GitHub repository: %s", exc)
            sys.exit(1)
    else:
    ...
    _section("Phase 6.5 Active Import Scan (Usage Check)")
    package_names = set(cve_to_pkg.values())
    import_scan_results = {}
    for pkg_name in sorted(package_names):
        scan_res = scan_package_usage(project_dir, pkg_name)
        import_scan_results[pkg_name] = scan_res.to_dict()
        print_stat_row(pkg_name, f"Used: {scan_res.is_used} ({scan_res.usage_count} occurrences)", style="green" if scan_res.is_used else "dim")
        
    import_scan_path = os.path.join(output_dir, "import_scan.json")
    with open(import_scan_path, "w", encoding="utf-8") as fh:
        json.dump(import_scan_results, fh, indent=2)
    ...
    _section("Phase 7.5 Real Sandbox Upgrade & AST Diff")
    all_api_changes = []
    sandbox_sim_results = {}
    
    # We retrieve targets if we have them
    targets = _pick_upgrade_targets(trivy_vulns, reachable_cves)
    if not targets:
        print_stat_row("Sandbox / AST", "No targets for dynamic sandbox check", style="yellow")
    else:
        for target in targets:
            pkg = target["package"]
            target_version = target["target_version"]
            print_stat_row("Sandbox install", f"{pkg}=={target_version}...", style="cyan")
            
            # Run real sandbox upgrade
            sandbox_res = await run_sandbox_upgrade(project_dir, pkg, target_version)
            sandbox_sim_results[pkg] = {
                "success": sandbox_res.success,
                "conflict_detected": sandbox_res.conflict_detected,
                "regression_passed": sandbox_res.regression_passed,
                "pip_check_output": sandbox_res.pip_check_output,
                "pip_log": sandbox_res.pip_log
            }
            
            if not sandbox_res.success:
                print_stat_row(pkg, "Sandbox installation failed", style="red")
                if not skip_llm and api_key:
                    try:
                        conflict_prompt = (
                            f"Review this failed pip installation log for package '{pkg}' upgraded to '{target_version}'.\n"
                            f"pip check output:\n{sandbox_res.pip_check_output}\n\n"
                            f"pip logs:\n{sandbox_res.pip_log[:4000]}\n\n"
                            "Provide a concise, developer-friendly explanation of why this dependency upgrade failed."
                        )
                        from src.ai_remediation import _call_gemini_sync
                        explanation = await asyncio.to_thread(_call_gemini_sync, api_key, conflict_prompt)
                        sandbox_sim_results[pkg]["conflict_explanation"] = explanation
                    except Exception as ex:
                        log.warning("Failed to get LLM explanation for dependency conflict: %s", ex)
                continue
                
            if sandbox_res.conflict_detected:
                print_stat_row(pkg, "Sandbox conflict detected", style="yellow")
                if not skip_llm and api_key:
                    try:
                        conflict_prompt = (
                            f"Review this pip check conflict output for package '{pkg}' upgraded to '{target_version}'.\n"
                            f"pip check output:\n{sandbox_res.pip_check_output}\n\n"
                            "Provide a concise, developer-friendly explanation of why this dependency upgrade conflict occurred."
                        )
                        from src.ai_remediation import _call_gemini_sync
                        explanation = await asyncio.to_thread(_call_gemini_sync, api_key, conflict_prompt)
                        sandbox_sim_results[pkg]["conflict_explanation"] = explanation
            ...
            print_stat_row(pkg, "Sandbox success!", style="green")
            
            # AST Diff
            if sandbox_res.old_source_dir and sandbox_res.new_source_dir:
                print_stat_row(pkg, "Extracting AST changes...", style="cyan")
                old_map = build_api_map(sandbox_res.old_source_dir)
                new_map = build_api_map(sandbox_res.new_source_dir)
                pkg_api_changes = diff_packages(old_map, new_map)
                all_api_changes.extend(pkg_api_changes)
                print_stat_row(pkg, f"Detected {len(pkg_api_changes)} API changes", style="green")
                
                # Clean up copied directories
                shutil.rmtree(sandbox_res.old_source_dir, ignore_errors=True)
                shutil.rmtree(sandbox_res.new_source_dir, ignore_errors=True)
                
    # Save sandbox simulation details to a file
    sandbox_sim_path = os.path.join(output_dir, "sandbox_sim_results.json")
    with open(sandbox_sim_path, "w", encoding="utf-8") as fh:
        json.dump(sandbox_sim_results, fh, indent=2)

    # Save ast_diffs.json
    serialized_api_changes = [c.to_dict() for c in all_api_changes]
    ast_diffs_path = os.path.join(output_dir, "ast_diffs.json")
    with open(ast_diffs_path, "w", encoding="utf-8") as fh:
        json.dump(serialized_api_changes, fh, indent=2)
        
    # Smart Code Scan
    print_stat_row("Smart Scan", f"Scanning project codebase for {len(all_api_changes)} API changes...", style="cyan")
    code_usages = scan_project_usages(project_dir, all_api_changes)
    serialized_code_usages = [u.to_dict() for u in code_usages]
    code_usages_path = os.path.join(output_dir, "code_usages.json")
    with open(code_usages_path, "w", encoding="utf-8") as fh:
        json.dump(serialized_code_usages, fh, indent=2)
    print_stat_row("Smart Scan", f"Found {len(code_usages)} usages of changed/removed APIs", style="green")
    
    # Remediation Generation
    print_stat_row("Remediation", "Generating code refactoring items...", style="cyan")
    try:
        remediation_plan = await generate_remediation(all_api_changes, code_usages)
    except Exception as e:
        log.warning("AI remediation failed: %s. Using deterministic fallback.", e)
        from src.ai_remediation import get_deterministic_remediation
        remediation_plan = get_deterministic_remediation(all_api_changes, code_usages)
        
    remediation_plan_path = os.path.join(output_dir, "remediation_plan.json")
    with open(remediation_plan_path, "w", encoding="utf-8") as fh:
        json.dump(remediation_plan.to_dict(), fh, indent=2)
    print_stat_row("Remediation", f"Generated remediation items: {len(remediation_plan.items)} (mode: {remediation_plan.generated_by})", style="green")
...
def main() -> None:
    ...
    p.add_argument("--project-dir", default=None, help="Target project to analyze")
    p.add_argument("--github-url", default=None, help="GitHub repository URL to clone and analyze")
    ...
    if args.project_dir is not None and args.github_url is not None:
        p.error("arguments --project-dir and --github-url are mutually exclusive")
    if args.project_dir is None and args.github_url is None:
        args.project_dir = "."
