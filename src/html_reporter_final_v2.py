...
def render_html(
    ...
    *,
    ...
    # Load and append new AST-based elements if they exist in output_dir
    ...
    data["api_changes"] = _load_json(Path(ast_diffs_path)) if os.path.exists(ast_diffs_path) else []
    data["code_usages"] = _load_json(Path(code_usages_path)) if os.path.exists(code_usages_path) else []
    data["remediation_plan"] = _load_json(Path(remediation_plan_path)) if os.path.exists(remediation_plan_path) else {}
    data["import_usages"] = _load_json(Path(import_scan_path)) if os.path.exists(import_scan_path) else {}
    data["sandbox_sim_results"] = _load_json(Path(sandbox_sim_path)) if os.path.exists(sandbox_sim_path) else {}
